...
namespace notePadProj
{
    ...
    public class CWord
    {
        protected Word._Application wordApp;//word应用程序变量
        protected Word._Document wordDoc;//word文档变量
        ...
        protected bool SaveWord(string filePath)
        {
            object fileName = filePath;
            wordDoc.SaveAs(ref fileName, ref oMissing, ref oMissing, ref oMissing, ref oMissing,
                    ref oMissing, ref oMissing, ref oMissing, ref oMissing, ref oMissing, ref oMissing,
        ...
        protected void OpenTemplate(string wordTemplate)
        {
            wordApp = new Application();
            object fileName = wordTemplate;
            wordDoc = wordApp.Documents.Add(ref fileName, ref oMissing, ref oMissing, ref oMissing);
        }
        //protected void OpenWordFile(string filePath)
        //{
        //    wordApp = new Application();
        //    object fileName = filePath;
        //    wordApp.Documents.Open(fileName, ref oMissing, ref oMissing, ref oMissing, ref oMissing,
        //        ref oMissing, ref oMissing, ref oMissing, ref oMissing, ref oMissing, ref oMissing,
        //        ref oMissing, ref oMissing, ref oMissing, ref oMissing, ref oMissing);
        //    //wordDoc = wordApp.Documents.Add(ref fileName, ref oMissing, ref oMissing, ref oMissing);
        //    wordApp.Visible = true;
    ...
    public class CWordProj : CWord
    {
        private CProject proj;
        private List<CProject> projList;//导出事项列表
        public CWordProj(CProject proj)
        {
            this.proj = proj;
        }
        public CWordProj(List<CProject> projList)
        {
            this.projList = projList;
        }
        public bool SaveProj2Word(string filePath)
        {
            try
            {
                //1.创建文件 
                ...
                para = wordDoc.Content.Paragraphs.Add(ref oMissing);
                //3.1插入标题
                para.Range.Text = "1.事项名称：" + proj.ProjectName;
                para.Range.Font.Size = 20f;
                //para.Range.Font.Bold = 2;
                //para.Range.Font.Color = WdColor.wdColorRed;
                //para.Range.Font.Italic = 2;
                //3.2插入创建日期、工程日期、完成日期、内容、是否完成
                para.Range.InsertParagraphAfter();
                para.Range.Font.Size = 14f;
                para.Range.Text = "2.创建日期：" + proj.CreateDate;
                para.Range.InsertParagraphAfter();
                para.Range.Text = "3.事项日期：" + proj.ProjectDate;
                para.Range.InsertParagraphAfter();
                if (proj.IsFinished)
                {
                    para.Range.Text = "4.完成日期：" + proj.FinishDate;
                }
                else
                {
                    para.Range.Text = "4.当前状态：未完成。";
                }
                para.Range.InsertParagraphAfter();
                para.Range.Text = "5.备注：" + proj.Content;
                para.Range.InsertParagraphAfter();
                //3.3插入附件信息表格wordApp.Selection.Range
                //换行填加表格
                gotoLastLine(wordDoc);
                List<CAttachment> attList = proj.GetAttList();
                Word.Table newTable = wordDoc.Tables.Add(wordApp.Selection.Range, attList.Count + 1, 3, ref oMissing, ref oMissing);
                //设置表格
                newTable.Borders.OutsideLineStyle = Microsoft.Office.Interop.Word.WdLineStyle.wdLineStyleSingle;
                newTable.Borders.InsideLineStyle = Microsoft.Office.Interop.Word.WdLineStyle.wdLineStyleSingle;
                newTable.Range.Font.Size = 14f;
                newTable.Range.Font.Bold = 0;
                newTable.Columns[1].Width = 40f;
                newTable.Columns[2].Width = 240f;
                newTable.Columns[3].Width = 100f;
                //填充表格内容
                //填充标题行
                newTable.Cell(1, 1).Range.Text = "序号";
                newTable.Cell(1, 2).Range.Text = "附件名称";
                newTable.Cell(1, 3).Range.Text = "填加时间";
                for (int i = 0; i < attList.Count; i++)
                {
                    newTable.Cell(i + 2, 1).Range.Text = (i + 1).ToString();
                    newTable.Cell(i + 2, 2).Range.Text = attList[i].AttName;
                    newTable.Cell(i + 2, 3).Range.Text = attList[i].UploadDate;
                }
                //垂直居中
                wordApp.Selection.Cells.VerticalAlignment = Microsoft.Office.Interop.Word.WdCellVerticalAlignment.wdCellAlignVerticalCenter;
                //水平居中
        ...
        private void gotoLastLine(_Document thisDocument)
        {
        ...
        /// <returns></returns>
        public bool SaveProjByModel(string savedFileName)
        {
            try
            {
                //1.打开模板进行文档初始化
                string wordTemplate = System.Windows.Forms.Application.StartupPath + "\\data\\model\\wordModel.docx";
                if (File.Exists(wordTemplate))
                {
                    OpenTemplate(wordTemplate);
                }
                else
                {
                    return false;
                ...
                //2.遍历书签，进行数据插入操作
                List<CAttachment> attList = proj.GetAttList();//附件信息
                Microsoft.Office.Interop.Word.Paragraph para = wordDoc.Content.Paragraphs.Add(ref oMissing);//段落
                foreach (Bookmark bm in wordDoc.Bookmarks)
                {
                    switch (bm.Name)
                    {
                        case "title":
                            bm.Select();
                            bm.Range.Text = proj.ProjectName;
                            break;
                        case "createdate":
                            bm.Select();
                            bm.Range.Text = proj.CreateDate;
                            break;
                        case "projdate":
                            bm.Select();
                            bm.Range.Text = proj.ProjectDate;
                            break;
                        case "status":
                            bm.Select();
                            bm.Range.Text = proj.IsFinished ? "已完成" : "未完成";
                            break;
                        case "finishdate":
                            bm.Select();
                            bm.Range.Text = proj.IsFinished ? proj.FinishDate : "--";
                            break;
                        case "content":
                            bm.Select();
                            bm.Range.Text = proj.Content;
                            break;
                        case "picture":
                            //bm.Select();
                            //图片标题
                            //3.1插入标题
                            wordDoc.Paragraphs.Last.Alignment = Microsoft.Office.Interop.Word.WdParagraphAlignment.wdAlignParagraphCenter;//设置格式居中
                            foreach (CAttachment att in attList)
                            {
                                if (FormAttachment.IsFilePicture(att.AttFilePath))
                                {
                                    //移动光标文档末尾
                                    object LinkToFile = false;
                                    object SaveWithDocument = true;
                                    object Anchor = wordDoc.Paragraphs.Last.Range;
                                    wordDoc.Application.ActiveDocument.InlineShapes.AddPicture(att.AttFilePath, ref LinkToFile, ref SaveWithDocument, ref Anchor);
                                    //插入图片名称
                                    wordDoc.Paragraphs.Last.Range.InsertParagraphAfter();
                                    wordDoc.Paragraphs.Last.Range.Text = att.AttName;
                                    wordDoc.Paragraphs.Last.Range.InsertParagraphAfter();
                ...
                {
                    Table tb = wordDoc.Tables[1];//获取表格对象
                    //增加新行
                    int rowindex = 1;
                    foreach (CAttachment att in attList)
                    {
                        //object beforeRow = tb.Rows[0];
                        tb.Rows.Add(ref oMissing);
                        tb.Cell(rowindex + 1, 1).Range.Text = rowindex.ToString();
                        tb.Cell(rowindex + 1, 2).Range.Text = att.AttName;
                        tb.Cell(rowindex + 1, 3).Range.Text = att.UploadDate;
                        rowindex++;
                    }
                    //删除最后一行空行
                    if (tb.Rows.Count > attList.Count + 1)
                    {
                        tb.Rows[attList.Count + 2].Delete();
                    }
                }
                //保存
                SaveWord(savedFileName);
                return true;
            }
            catch (Exception e)
            {
        ...
        /// <param name="savedFileName"></param>
        /// <returns></returns>
        public bool SaveProjListByModel(string savedFileName)
        {
            try
            {
                //1.打开模板进行文档初始化
                string wordTemplate = System.Windows.Forms.Application.StartupPath + "\\data\\model\\DataModel.docx";
                if (!File.Exists(wordTemplate))
                {
                    CDBOperate.CreateWordModalFile(wordTemplate);
                }
                if (File.Exists(wordTemplate))
                {
                    OpenTemplate(wordTemplate);
                ...
                if (wordDoc.Tables.Count > 0)
                {
                    ...
                    //增加新行
                    int rowindex = 1;
                    foreach (CProject project in projList)
                    {
                        tb.Rows.Add(ref oMissing);
                        tb.Cell(rowindex + 1, 1).Range.Text = rowindex.ToString();//序号
                        tb.Cell(rowindex + 1, 2).Range.Text = project.ProjectName;
                        tb.Cell(rowindex + 1, 3).Range.Text = project.CreateDate;
                        tb.Cell(rowindex + 1, 4).Range.Text = project.ProjectDate;
                        tb.Cell(rowindex + 1, 5).Range.Text = project.IsFinished ? project.FinishDate : "未完成";
                        tb.Cell(rowindex + 1, 6).Range.Text = project.Content;
                        rowindex++;
                    }
                    //删除最后一行空行
                    if (tb.Rows.Count > projList.Count + 1)
                    {
                        tb.Rows[projList.Count + 2].Delete();
                    }
                }
                //保存
                if (SaveWord(savedFileName))
                {
                    if (System.Windows.Forms.MessageBox.Show("导出完成") == System.Windows.Forms.DialogResult.OK)
                    {
                        wordApp.Visible = true;
                    }
                    return true;
